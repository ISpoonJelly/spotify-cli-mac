...
const setTokens = () => {
  ...
  let clientSecret = readlineSync.question(
    'What is your Spotify Client Secret? \n',
    { hideEchoBack: true },
  );
...
function setUsername() {
  let clientUsername = readlineSync.question(
          ...
          prompt.get(['selection'], function(err, result) {
            ...
            if (results[result.selection - 1]) {
              if (results[result.selection - 1].noOfTracks > 0) {
                ...
                printer.error(
                  'This playlist has no songs, so it could not be played',
                );
              }
            }
          });
...
program
  ...
  .description(
    'Search for a <track (t) | artist (ar) | album (al) | playlist (p) > (searches tracks by default)',
  )
  .alias('s')
  .action((type, query) => {
...
program
  .command('info')
  .alias('i')
  .description('Display meta-data about the current track')
  .action(() => {
    spotifyClient.status().then(result => {
...
program
  .command('play [uri]')
  .description(
    'Continue playing current track or play the track with the provided URI',
  )
  .action(uri => {
    if (uri) {
  ...
  .action(username => {
    if (username) {
      ...
      if (SPOTIFY_USERNAME_SET) {
        ...
        printer.warning(
          'You will need to set your username first using spotify username.',
        );
    ...
    spotifyApi.clientCredentialsGrant().then(
      function(data) {
        ...
        spotifyClient.getCurrentSongId().then(data => {
          spotifyApi
            .getRecommendations({ seed_tracks: [data] })
            .then(response => {
              ...
              prompt.get(['selection'], function(err, result) {
                ...
                if (results[result.selection - 1]) {
                  var selectedSpotifyURI =
                    results[result.selection - 1].spotifyURI;
                  spotifyClient.play(selectedSpotifyURI).then(() => {
                    spotifyClient.status().then(result => {
                      printer.printPlayerStatus(result);
                    });
 ...
 * */
let publishedVersion = require('child_process').execSync(
  `npm info spotify-cli-mac version`,
);
...
if (semver.lt(version, semver.clean(publishedVersion))) {
  console.log(
    `Your Spotify CLI is outdated. Latest version is ${publishedVersion}, you're on ${version}`,
  );
