...
function printDurationProgress(result) {
  var statusButton =
    result.status === 'playing' ? ':arrow_forward:' : ':double_vertical_bar:';
  statusButton = emoji.emojify(statusButton);
  new ProgressBar(
    `${statusButton}  ${result.status} [:bar] ${result.position} of ${result.duration}`,
    {
      complete: '=',
      incomplete: ' ',
      width: 50,
...
function printPlayerStatus(result) {
  var artist = `:microphone:  ${chalk.green('Artist:')} ${chalk.green(
    result.artist,
  )}`;
  var track = `:musical_score:  ${chalk.green('Track:')} ${chalk.green(
    result.track,
  )}`;
...
function printNext(result) {
  var nextTrack = `:fast_forward:  Playing next track: ${chalk.green(
    result.track,
  )} :musical_score:`;
...
function printPrevious(result) {
  var previousTrack = `:rewind:  Playing previous track: ${chalk.green(
    result.track,
  )} :musical_score:`;
...
function printToggleShuffle(status) {
  var description = status
    ? `${chalk.green('turned ON.')}`
    : `${chalk.red('turned OFF.')}`;
...
function printConfig() {
  console.log(chalk.green('Config set!'));
}
